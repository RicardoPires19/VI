...
function data_table(sample) {
	...
	.enter()
		...
		.on("mouseover", function(d){
			...
			if(selected_movie == null) highlight(d);
		})
		.on("mouseout", function(d){
			// only unhighlights if there is not a selected movie
			if(selected_movie == null) unhighlight(d);
		})
		.on("click", function(d){ 
			// if there is not a selected movie
			if(selected_movie == null) {
				// highlight the new selection
				selected_movie = d;	
				highlight(selected_movie);
			}
			// if you want to select a diferent movie
			else if(selected_movie != d) {
					// unhilights the previous selection
					unhighlight(selected_movie);
					// highlights the new one
					selected_movie = d;
					highlight(selected_movie);
				} else {
					// if you want to unselect the movie
					unhighlight(selected_movie);	
					selected_movie = null;
			}
		})
	...
	.append("span")
		.text(function(d)
		{
			if(d["originalTitle"] == 0 )
				return d["title"]
			else return d["title"] + " (" + d["originalTitle"] + ")";
		})
}
...
function paths(selected, ctx, count) {
	...
	data_table(shuffled_data.slice(0,6));
...
function search(selection,str) {
	...
	print_pattern = pattern.exec(data[10]["title"]);
